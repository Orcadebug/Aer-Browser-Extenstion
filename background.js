...
async function ensureAssistScripts(tabId, url) {
  ...
  // Inject content extraction and assist overlay on demand (no persistent host permissions)
  try {
    await chrome.scripting.executeScript({ target: { tabId }, files: ['content.js'] });
  } catch (e) { console.warn('[EnsureAssist] inject content.js failed:', e); }
  try {
  ...
  if (request.action === 'upload' || request.action === 'saveToAer') {
    (async () => {
      try {
        ...
        // If we only have a URL/title stub, enrich with page content
        try {
          ...
          if ((!dataToUpload.content || looksStub) && sender?.tab?.id) {
            ...
            try {
              page = await chrome.tabs.sendMessage(sender.tab.id, { action: 'extractContent' });
            } catch {}
            // Fallback to scripting execute if content script not present
            if (!page || !page.content) {
              try {
                page = await robustExtractPage(sender.tab.id);
              } catch {}
            }
            if (page && page.content) {
...
async function robustExtractPage(tabId) {
  ...
  try {
    ...
    // Safe fallback when we cannot access the page context
